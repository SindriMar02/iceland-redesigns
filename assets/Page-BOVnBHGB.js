...
.vn-nav.is-ink::before {
  background: linear-gradient(180deg, ${K} 0%, color-mix(in srgb, ${K} 88%, transparent) 45%, color-mix(in srgb, ${K} 34%, transparent) 78%, transparent 100%);
}
.vn-nav-mark {
...
.vn-ht-stack { position: relative; display: block; aspect-ratio: 4 / 5; overflow: hidden; background: ${Z}; }
...
.vn-hero-tours:hover .vn-ht-cta {
  background: #F2F1EE; border-color: #F2F1EE; color: ${Z}; transform: translateX(3px);
}
...
.vn-foot-wordmark {
  margin: 0; font-family: ${M}; font-weight: 700; line-height: .96;
...
.vn-loader {
  position: fixed; inset: 0; z-index: 60; background: ${K};
